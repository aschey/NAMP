...
impl Player {
    ...
    pub async fn set_volume_temp(&mut self, volume: f32) {
        if let Some(src) = self.sources.get(0) {
            info!("set volume src {:?}", src.nodes.gain);
            let gain = src.nodes.gain;
            call!(self.context.set_volume(vec![gain], volume))
                .await
                .unwrap();
        } else {
            info!("no src");
    ...
    pub async fn load(
        ...
        let nodes = call!(self
            .context
            .create_nodes(buffer_source_id, gain_init, analyser_init))
        .await
        .unwrap();
        ...
        if self.sources.len() == 0 {
            ...
            let seconds = prev.start_time + prev_duration.unwrap_or_default() - start_gap - 0.03; //(prev.duration - prev.end_gap - file_info.start_gap) - 0.03;
            start_time = seconds;
        ...
        self.sources.push_back(ScheduledSource {
            path,
            start_time,
            end_gap: 0.,  //file_info.end_gap,
            duration: 0., //computed_duration,
            nodes,
        });
    }
